...
export const sortPostsByDate = (posts, newestFirst = true) => {
  return [...posts].sort((a, b) => {
    // Prioritize the date field over createdAt for better sorting
    const dateA = parseDate(a.date || a.createdAt);
    const dateB = parseDate(b.date || b.createdAt);
    
    console.log(`Sorting: "${a.title}" (${a.date || a.createdAt}) -> ${dateA.toISOString()}`);
    console.log(`Sorting: "${b.title}" (${b.date || b.createdAt}) -> ${dateB.toISOString()}`);
